...
export function registerCommands(pi: ExtensionAPI, deps: CommandDeps): void {
  ...
  cmd(
    "doc-inject",
    "Doc injector: on|off|toggle|list|reset|status",
    async (args, ctx) => {
      ...
      if (a === "on") {
        ...
        ctx.ui.notify(
          `📄 Doc injection ${next ? "enabled" : "disabled"}`,
          "info",
        );
      } else if (a === "reset") {
  ...
  cmd(
    "doc-keywords-gen",
    "Generate LLM keywords: /doc-keywords-gen [path] — no arg = all keyword-less files",
    async (args, ctx) => {
      const reg = deps.getRegistry();
      if (!reg) {
        ctx.ui.notify("📄 No registry loaded", "warning");
      ...
      if (!config.llmKeywords) {
        ctx.ui.notify(
          "📄 LLM keyword generation is disabled (llmKeywords: false in config)",
          "warning",
        );
        return;
      }
      ...
      if (targetPath) {
        candidates = candidates.filter((e) =>
          e.relativePath.includes(targetPath),
        );
        if (candidates.length === 0) {
          ctx.ui.notify(
            `📄 No keyword-less files matching "${targetPath}"`,
            "info",
          );
          return;
        }
      ...
      const batches: Array<
        Array<{ path: string; snippet: string; existingKeywords: string[] }>
      > = [];
      ...
      ctx.ui.notify(
        `📄 Sending ${batches.length} keyword-generation batch(es) for ${candidates.length} file(s)...`,
        "info",
      );
