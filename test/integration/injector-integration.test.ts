...
interface MockExtensionAPI {
  ...
  registerTool: (def: {
    ...
    description: string;
    parameters: unknown;
    execute: (...args: unknown[]) => Promise<MockToolResult>;
  }) => void;
  ...
  _commandHandlers: Map<
    string,
    (
      args: string,
      ctx: { ui: { notify: ReturnType<typeof createMockFn> } },
    ) => Promise<void>
  >;
...
const triggerAssistantStream = async (
...
describe("Doc Injector Extension - Integration", () => {
  ...
  test("injects docs when keywords match in streaming output", async () => {
    ...
    expect(typedResult?.message?.content).toContain(
  ...
  test("matches keywords found in assistant thinking blocks", async () => {
    ...
    await triggerAssistantStreamBlocks(api, [
      {
        type: "thinking",
        text: "The user is asking about testing their code.",
      },
  ...
  test("does not inject when keywords appear only in non-thought text", async () => {
    ...
    const result = await triggerAgentStart(api, "You are a helpful assistant.");
    expect(
      (result as { message?: { content: string } } | undefined)?.message,
    ).toBeUndefined();
  });
  ...
  test("injection is gated by matchThreshold in the full flow", async () => {
    ...
    expect(
      (r1 as { message?: { content: string } } | undefined)?.message,
    ).toBeUndefined();
  ...
  test("marks injected docs so they aren't re-injected", async () => {
    ...
    expect(
      (result1 as { message?: { content: string } })?.message?.content,
    ).toContain("Testing Guide");
...
describe("Doc Injector Extension - Commands", () => {
  ...
  test("/doc-inject reset allows re-injection", async () => {
    ...
    expect(
      (result2 as { message?: { content: string } } | undefined)?.message,
    ).toBeUndefined();
    ...
    const result3 = await triggerAgentStart(
      api,
      "You are a helpful assistant.",
    );
    expect(
      (result3 as { message?: { content: string } })?.message?.content,
    ).toContain("Testing Guide");
  });
  ...
  test("/doc-inject on/off enables and disables injection", async () => {
    ...
    expect(
      (result2 as { message?: { content: string } })?.message?.content,
    ).toContain("Testing Guide");
  });
});
...
`,
  ...
  test("reuses LLM-generated keywords after resources_discover reload", async () => {
    ...
    const notification = (notifyFn as unknown as { calls: unknown[][] })
      .calls[0][0] as string;
...
describe("Doc Injector Extension - Auto-Abort on Stream", () => {
  ...
  test("aborts when assistant mentions a new keyword mid-stream", async () => {
    ...
    const abortCalls = (
      api._sessionCtx.abort as unknown as { calls: unknown[][] }
    ).calls;
  ...
  test("agent_end sends follow-up message to restart after abort", async () => {
    ...
    const sendCalls = (api.sendUserMessage as unknown as { calls: unknown[][] })
      .calls;
...
describe("Doc Injector Extension - Multi-Chunk Stream Accumulation", () => {
  ...
  test("promotes doc when distinct keywords appear in different chunks' windows", async () => {
    ...
    await api.emit(
      "message_update",
      {
        message: {
          role: "assistant",
          content: "x".repeat(80) + " testing" + "x".repeat(80) + " bun tail",
        },
      },
      api._sessionCtx,
    );
    abortCalls = (api._sessionCtx.abort as unknown as { calls: unknown[][] })
      .calls;
    expect(abortCalls.length).toBe(1);
  });
  ...
  test("streamHits is cleared on message_end so the next message starts fresh", async () => {
    ...
    (api._sessionCtx.abort as unknown as { calls: unknown[][] }).calls.length =
      0;
...
describe("Doc Injector Extension - No Double Injection Validation", () => {
  ...
  test("same keyword across 10 turns produces exactly 1 injection", async () => {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      const result = await triggerAgentStart(
        api,
        "You are a helpful assistant.",
      );
  ...
  test("different keywords across turns each inject their doc exactly once", async () => {
    ...
    const c1 =
      (r1 as { message?: { content: string } } | undefined)?.message?.content ??
      "";
    ...
    const c2 =
      (r2 as { message?: { content: string } } | undefined)?.message?.content ??
      "";
    ...
    const c3 =
      (r3 as { message?: { content: string } } | undefined)?.message?.content ??
      "";
    ...
    const c4 =
      (r4 as { message?: { content: string } } | undefined)?.message?.content ??
      "";
  ...
  test("user message AND mid-stream assistant message both match an already-injected doc: no duplicate", async () => {
    ...
    const r2 = await triggerAgentStart(api, "x");
    expect(
      (r2 as { message?: { content: string } } | undefined)?.message,
    ).toBeUndefined();
  });
  ...
  test("auto-abort + restart: no duplicate injection across the restart", async () => {
    ...
    await api.emit(
      "message_update",
      {
        message: {
          role: "assistant",
          content: "let's also cover the workflow",
        },
      },
      api._sessionCtx,
    );
  ...
  test("re-injection is possible ONLY after /doc-inject reset, not before", async () => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const r = await triggerAgentStart(api, "x");
      expect(
        (r as { message?: { content: string } } | undefined)?.message,
      ).toBeUndefined();
    }
