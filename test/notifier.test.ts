...
describe("ExtensionNotifier", () => {
    test("buffers messages before setContext is called", () => {
        ...
        const calls = (ctx.ui.notify as unknown as { mock: { calls: unknown[][] } }).mock.calls;
    ...
    test("forwards messages directly once a context is bound", () => {
        ...
        const ctx2Calls = (ctx2.ui.notify as unknown as { mock: { calls: unknown[][] } }).mock.calls;
    ...
    test("does not re-flush already-flushed messages on a second setContext", () => {
        ...
        expect((ctx1.ui.notify as unknown as { mock: { calls: unknown[][] } }).mock.calls.length).toBe(1);

        // A second setContext (e.g. session reload) should NOT re-send the
        // already-flushed message.
        const ctx2 = makeCtx();
        notifier.setContext(ctx2);
        expect((ctx2.ui.notify as unknown as { mock: { calls: unknown[][] } }).mock.calls.length).toBe(0);
    });
    ...
    test("preserves arrival order across multiple levels", () => {
        ...
        expect(calls.map((c) => c[1])).toEqual(["info", "warning", "error", "info"]);
    });
});
