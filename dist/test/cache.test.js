...
describe("loadCache", () => {
    ...
    test("returns empty cache for wrong version", async () => {
        writeFileSync(join(TEST_DIR, ".pi", "doc-injector-cache.json"), JSON.stringify({ version: 2, files: {} }));
        const cache = await loadCache(TEST_DIR);
        expect(cache.version).toBe(1);
        expect(cache.files).toEqual({});
    });
    test("returns empty cache when version field is missing", async () => {
        writeFileSync(join(TEST_DIR, ".pi", "doc-injector-cache.json"), JSON.stringify({ files: { "test.md": { mtimeMs: 123, keywords: ["a"] } } }));
        const cache = await loadCache(TEST_DIR);
        expect(cache.version).toBe(1);
        expect(cache.files).toEqual({});
    });
    test("returns empty cache when files field is missing", async () => {
        writeFileSync(join(TEST_DIR, ".pi", "doc-injector-cache.json"), JSON.stringify({ version: 1 }));
        const cache = await loadCache(TEST_DIR);
        expect(cache.version).toBe(1);
        expect(cache.files).toEqual({});
    });
